...
// --- LOẠI BỎ IMPORT THƯ VIỆN GOOGLE GENAI ĐỂ GIẢM DUNG LƯỢNG VÀ BẢO MẬT ---
// Chúng ta sẽ định nghĩa lại Type Enum dạng string đơn giản để tạo Schema JSON
const Type = {
  OBJECT: 'OBJECT',
  ARRAY: 'ARRAY',
  STRING: 'STRING',
...
export const parseGeometryProblem = async (
  ...
  // Thay vì gọi API tại đây, ta gửi toàn bộ cấu hình sang Parent Iframe (AI Studio)
  return new Promise((resolve, reject) => {
      ...
      const handleMessage = (event: MessageEvent) => {
          ...
          if (event.data?.type === 'GEMINI_RESULT' && event.data?.requestId === requestId) {
              ...
              try {
                  ...
                  const result = typeof event.data.payload === 'string' ? JSON.parse(event.data.payload) : event.data.payload;
                  ...
                  reject(new Error("Lỗi xử lý dữ liệu từ AI Studio."));
              }
          }
      ...
      const timeoutId = setTimeout(() => {
          window.removeEventListener('message', handleMessage);
          reject(new Error("Quá thời gian chờ AI Studio phản hồi (60s). Hãy kiểm tra kết nối."));
      }, TIMEOUT);

      // GỬI LỆNH ĐI (Hồn gọi Da)
      // Gửi cấu hình đầy đủ để AI Studio chỉ việc thực thi
      window.parent.postMessage({
          type: 'DRAW_REQUEST',
          requestId,
          payload: {
              model: 'gemini-2.0-flash', // Model xịn xò
              systemInstruction: SYSTEM_INSTRUCTION,
              contents: { parts }, // Đã bao gồm ảnh và text
              responseSchema: RESPONSE_SCHEMA, // Schema định nghĩa ở trên
              thinkingBudget: 12000 // Config cho model mới
          }
      }, '*');
  });
};
